...
CREATE TABLE channels (
    id                      TEXT PRIMARY KEY,
    instance_id             TEXT REFERENCES instances(id) ON DELETE CASCADE,  -- NULL = local
...
-- Federation instance_id indexes (partial — only rows with instance_id set)
CREATE INDEX idx_channels_instance_id     ON channels(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_roles_instance_id        ON roles(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_guild_members_instance_id ON guild_members(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_messages_instance_id     ON messages(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_attachments_instance_id  ON attachments(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_embeds_instance_id       ON embeds(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_reactions_instance_id    ON reactions(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_pins_instance_id         ON pins(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_webhooks_instance_id     ON webhooks(instance_id) WHERE instance_id IS NOT NULL;
```
...
shorthand = ""                      # max 5 chars; auto-derived from domain if empty
voice_mode = "direct"               # direct | relay
peer_inbox_limit = 20
delivery_concurrency = 50
media_cache_dir = "/tmp/amityvox-media-cache"
media_cache_max_size_mb = 1024
